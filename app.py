        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data['status'] == 'live':
                    ...
                    print("Lỗi: Trạng thái token không phải 'live'")
            else:
...
async def token_updater():
    while True:
        await get_jwt_token()
        await asyncio.sleep(8 * 3600)  # Cập nhật mỗi 8 giờ
...
async def get_clan_info(clan_id: str):
    global jwt_token
    if not jwt_token:
        raise HTTPException(status_code=500, detail="JWT token bị thiếu hoặc không hợp lệ")
    if not clan_id:
        raise HTTPException(status_code=400, detail="Yêu cầu cung cấp Clan ID")
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            response = await client.post(url, headers=headers, content=encrypted_data)
        except httpx.RequestError as e:
            raise HTTPException(status_code=500, detail=f"Lỗi yêu cầu API: {str(e)}")
    ...
    if response.status_code == 200:
        if response.content:
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Lỗi phân tích phản hồi protobuf: {str(e)}")
        else:
            raise HTTPException(status_code=500, detail="Không có nội dung trong phản hồi")
    else:
        raise HTTPException(status_code=response.status_code, detail=f"Lấy dữ liệu thất bại: Mã trạng thái {response.status_code}")